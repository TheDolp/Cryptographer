...
class VigenereCode:
	...
	def code_str(self, str_in, code):
		return ''.join([self.alphabet.move_symb(char, self.alphabet.get_number(code_char)) 
						if self.alphabet.is_in_alph(char) else char for char, code_char in zip(str_in, code)])
